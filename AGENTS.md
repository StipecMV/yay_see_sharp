...
| Rozhodnutie | Stav |
|---|---|
| MVP backend = `yay`; `paru` je budúci extension point, NIE MVP | ✅ potvrdené |
...
- **`paru` ako MVP backend** — ZAMIETNUTÉ pre MVP. Persisted `PackageManagerEngine.Paru` preference sa pri load **clampne späť na yay** (regresný test existuje). Detect hlási „paru found (not supported yet)".
